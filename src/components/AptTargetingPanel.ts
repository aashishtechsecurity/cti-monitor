...
export class AptTargetingPanel {
  ...
  private selectedCountry: string = 'IN';
  private aptData: AptGroup[] = [];
  private onSelectCallback?: (countryCode: string) => void;

  constructor(containerId: string, onSelectCallback?: (countryCode: string) => void) {
    ...
    container.querySelectorAll('.country-btn').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        ...
        if (code) {
          ...
          if (this.onSelectCallback) this.onSelectCallback(code);
          this.render();
        }
      });
    });
